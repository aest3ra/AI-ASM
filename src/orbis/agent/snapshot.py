...
(maxRefs) => {
    ...
    for (const old of Array.from(document.querySelectorAll("[data-orbis-ref]"))) {
        old.removeAttribute("data-orbis-ref");
    }
    const refs = [];
    const blockedBy = (el) => {
...
async def capture_agent_snapshot(page: Page, *, max_refs: int = 120) -> dict[str, Any]:
    ...
    The script writes temporary `data-orbis-ref` attributes that the tool
