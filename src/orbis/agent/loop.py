...
from orbis.agent.budget import BudgetExceeded
from orbis.agent.client import LLMClient
from orbis.agent.safety import safe_tool_arguments
...
class AgentLoop:
    ...
    async def run_page(
        ...
        *,
        ...
        after_action: Callable[[int, ToolCall, ToolResult, str], Awaitable[bool | None]],
        max_turns: int,
        ...
        for turn in range(max_turns):
            ...
            page_url = str(context.get("page_url") or self.page_url or "")
    ...
    async def _execute_batch(
        ...
        *,
        ...
        for call in calls[:4]:
            ...
            should_continue = await after_action(turn, call, result, source)
            if (
                call.name == "give_up"
