...
from orbis.config import DEFAULT_AGENT_MODEL, DEFAULT_AGENT_TEMPERATURE
...
from orbis.agent.tools import TOOL_SPECS, ToolCall
...
def _make_openai_client(api_key: str | None):
    ...
    if not resolved:
        raise RuntimeError(
            "OPENAI_API_KEY is required for agent.mode='llm' or 'hybrid'. "
...
def _default_system_prompt() -> str:
    path = Path(__file__).parent / "prompts" / "orbis_agent.md"
    try:
