...
from orbis.agent.client import load_openai_api_key
from orbis.agent.behavior import analyze_trace_file
from orbis.analyzer.dispatcher import AnalyzerDispatcher
...
from orbis.crawler.runner import Crawler
from orbis.crawler.scope import Scope
from orbis.crawler.types import CapturedRequest, ScanDiagnostics
from orbis.normalizer import (
    ApiCandidate,
    NormalizedEndpoint,
    discover_api_candidates,
    normalize,
)
from orbis.normalizer.pipeline import canonical_api_path, is_api_capture
from orbis.normalizer.url import templatize_path
from orbis.output.artifacts import write_capture_artifact
from orbis.output.request_log import write_request_log
from orbis.schema.inferrer import infer_schema_from_json_bodies
...
from orbis.storage.db import FlaggedItem, FrontierState, Scan, open_db
...
class ScanOrchestrator:
    ...
    def run(self) -> ScanRunResult:
        ...
        if self.config.agent.mode in {"hybrid", "llm"} and not load_openai_api_key():
            raise RuntimeError(
                f"OPENAI_API_KEY is required for agent.mode='{self.config.agent.mode}'. "
        ...
        trace_path = self.out_dir / f"orbis-trace-{scan_id}.jsonl"
        ...
        raw_path = self.out_dir / f"orbis-scan-{scan_id}-{int(time.time())}.json"
        write_capture_artifact(raw_path, captured)
        request_log_path = self.out_dir / f"orbis-requests-{scan_id}.jsonl"
    ...
    def _finish_scan(
        ...
        agent_behavior = analyze_trace_file(trace_path) if trace_path.exists() else None
        cache_hit_input = (
            agent_behavior.cache_read_input_tokens if agent_behavior is not None else 0
        )
        tokens_input = agent_behavior.input_tokens if agent_behavior is not None else 0
        tokens_output = agent_behavior.output_tokens if agent_behavior is not None else 0
        cache_hit_rate = cache_hit_input / tokens_input if tokens_input else 0.0
        with Session(engine) as session:
            scan_row = session.get(Scan, scan_id)
            if scan_row is None:
