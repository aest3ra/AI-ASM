...
from ai_asm.crawler.types import FormStats, InteractionStats
...
async (payload) => {
  ...
  const form = document.querySelector(`[data-ai-asm-ref="${ref}"]`);
...
async def run_agent_interactions(
    ...
    *,
    ...
    async def before_action(turn: int, call: ToolCall) -> None:
        nonlocal before_cursor
        if network_events is not None:
            before_cursor = network_events.cursor()
            return
        before = before_observation or await observe_agent_state(page)
        before_cursor = len(before.requests)
    ...
    async def after_action(
        ...
        delta = _network_delta(before, after, new_records=new_records)
        record = ActionRecord(
            turn=turn,
            tool=call.name,
...
def _network_delta(
    ...
    *,
    ...
    api_records = _api_request_records(new_records)
    return NetworkDelta(
