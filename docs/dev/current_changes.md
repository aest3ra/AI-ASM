...
- Phase 6 출력 계층이 추가됐다.
  ...
  - scan 종료 후 OpenAPI 3.0 YAML을 자동 생성한다.
  - scan 종료 후 flagged item을 `yaml`, `curl` 형식으로 자동 생성한다.
  - schema 병합은 `integer + number`를 `number`로 합치고, 과도한 mixed `oneOf`를 축소한다.
- 기본 scan 저장 정책을 변경했다.
  - `--db`가 없으면 매 실행마다 `runs/orbis-<timestamp>_<host-slug>_<urlhash6>.db`를 새로 만든다.
  - artifact 디렉토리는 DB stem으로 자동 결정한다.
...
- 전체 테스트:
  ...
  - 결과: `272 passed`
- 자동 DB 경로 smoke:
  - `--db` 없이 `/tmp`에서 실제 URL 스캔
  - 생성 예: `runs/orbis-20260509-125418_careers-bancoplata-mx_7bff30.db`
- 자동 output smoke:
...
- Phase 6 이후 OpenAPI/flagged output 단위 테스트와 CLI smoke 테스트를 추가했다.

## 남은 과제
- dry-run으로 캡처한 mutation endpoint를 output/flagged 관점에서 더 명확히 표시할지 결정 필요.
